...
class GecBERTModel(object):
    ...
    def _get_embbeder(self, weigths_name, special_tokens_fix):
        embedders = {
            "bert": PretrainedBertEmbedder(
                pretrained_model=weigths_name, requires_grad=False, top_layer_only=True, special_tokens_fix=special_tokens_fix
            )
        }
        text_field_embedder = BasicTextFieldEmbedder(
            token_embedders=embedders, embedder_to_indexer_map={"bert": ["bert", "bert-offsets"]}, allow_unmatched_keys=True
        )
        return text_field_embedder

    def _get_indexer(self, weights_name, special_tokens_fix):
        bert_token_indexer = PretrainedBertIndexer(
            pretrained_model=weights_name, do_lowercase=self.lowercase_tokens, max_pieces_per_token=5, special_tokens_fix=special_tokens_fix
        )
    ...
    def update_final_edit(self, final_batch, final_edit, pred_ids, pred_batch, prev_preds_dict):
        """
        copy of update_final_batch, edit version.
        """
        new_pred_ids = []
        for i, orig_id in enumerate(pred_ids):
            orig = final_batch[orig_id]
            ...
            if orig != pred and pred not in prev_preds:
                # final_batch[orig_id] = pred
                final_edit[orig_id].append(edits)
                new_pred_ids.append(orig_id)
                prev_preds_dict[orig_id].append(pred)
            elif orig != pred and pred in prev_preds:
                # update final batch, but stop iterations
    ...
    def postprocess_batch(self, batch, all_probabilities, all_idxs, error_probs):
        ...
        for tokens, probabilities, idxs, error_prob in zip(batch, all_probabilities, all_idxs, error_probs):
            ...
            for i in range(length + 1):
                ...
                edits.append(action)
            all_results.append((get_target_sent_by_edits(tokens, edits), edits))
        return all_results
    ...
    def handle_batch(self, full_batch):
        ...
        final_edit = [[] for _ in range(batch_size)]
        ...
        for n_iter in range(self.iterations):
            ...
            _final_batch = copy.deepcopy(final_batch)  # saved for update_final_edit
            _pred_ids = copy.deepcopy(pred_ids)  # saved for update_final_edit
            final_batch, pred_ids, cnt = self.update_final_batch(final_batch, pred_ids, pred_batch, prev_preds_dict)
            final_edit = self.update_final_edit(_final_batch, final_edit, _pred_ids, pred_batch, prev_preds_dict)
            total_updates += cnt

            if not pred_ids:
                break

        return final_batch, final_edit, total_updates
