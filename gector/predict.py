...
def main(args):
    ...
    if args.input_dir is None:
        ...
        assert args.input_dir.endswith("/"), args.input_dir
        assert args.output_dir is not None, args.output_dir
        assert args.output_dir.endswith("/"), args.output_dir
        input_files = list(glob.glob(f"{args.input_dir}*.txt"))
        for input_file in tqdm(input_files, desc="GEC inference", total=len(input_files)):
            file_name = input_file.split("/")[-1]
            output_file = f"{args.output_dir}{file_name}"
            assert os.path.exists(input_file)
            assert not os.path.exists(output_file)
            predict_for_file(input_file, output_file, model,
                            batch_size=args.batch_size, 
                            to_normalize=args.normalize)
